...
export class Web4FeatureFlagsManager {
  ...
  private initializeDefaultFlags(): void {
    const defaultFlags: Web4FeatureFlag[] = [
      ...
      // NFT Envelopes and Public Directory
      {
        id: 'nft_envelopes',
        name: 'NFT Envelopes',
      ...
      {
        id: 'presence_ghost_mode_hard_off',
        name: 'Presence Ghost Mode',
        description: 'Hard off switch for all presence features',
        category: 'privacy',
        enabled: false,
        defaultValue: false,
        isSafeToToggle: true,
...
export const featureFlags = {
  ...
  isFlagEnabled: (flagId: string) => getFeatureFlagsManager().isFlagEnabled(flagId),
  setFlag: (flagId: string, enabled: boolean, reason?: string) => getFeatureFlagsManager().setFlag(flagId, enabled, reason)
};
